<!DOCTYPE html>
<html lang="es" data-theme="nord">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="<?php echo BASE_URL; ?>assets/css/output.css">
  <title><?php echo isset($page_title) ? $page_title . ' - Lugo tech' : 'Lugo tech - Cursos online'; ?></title>
  <link rel="icon" type="image/png" href="<?php echo BASE_URL; ?>assets/img/logo.png" />

</head>

<body>
  <?php require_once __DIR__ . '/error.php'; ?>

  <!-- Header Principal -->
  <header class="sticky top-0 z-50 w-full backdrop-blur-md bg-base-100/70 border-b-3 border-black ">
    <nav class="navbar bg-base-100/90 max-w-7xl mx-auto px-4 border-none">

      <!-- Logo y Menu Mobile -->
      <div class="navbar-start">
        <div class="dropdown">
          <label tabindex="0" class="btn btn-ghost btn-sm border-black lg:hidden">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h8m-8 6h16" />
            </svg>
          </label>
          <ul tabindex="0" class="menu menu-sm dropdown-content mt-3 z-10 p-2 shadow-lg bg-base-100 rounded-box w-52 gap-1">
            <li>

              <a href="<?= BASE_URL ?>index.php" class="font-display font-semibold">
                <i data-lucide="home" class="size-4"></i>
                Inicio
              </a>

            </li>
            <li>
              <a href="<?= BASE_URL ?>pages/coursesPublic.php" class="font-display font-semibold">
                <i data-lucide="book" class="size-4"></i>
                Cursos
              </a>

            </li>

            <?php if (isset($_SESSION['user'])): ?>
              <li>
                <a href="<?= BASE_URL ?>pages/studentCourses.php" class="font-display font-semibold">
                  <i data-lucide="graduation-cap" class="size-4"></i>
                  Mi aprendizaje
                </a>
              </li>
            <?php endif; ?>
          </ul>
        </div>

        <!-- Logo -->
        <a href="<?= BASE_URL ?>index.php" class="flex items-center normal-case gap-2 px-2 lg:px-4 ">
          <div class="avatar">
            <div class="w-8 h-8 bg-primary border-3 border-black flex items-center justify-center shadow-neo-sm">
              <img src="<?= BASE_URL ?>assets/img/logo.png" alt="">
            </div>
          </div>
          <div class="flex flex-col items-start">
            <span class="font-display font-bold text-lg tracking-tight text-base-content hidden sm:inline">Lugo Tech</span>
          </div>
        </a>
      </div>

      <!-- Menu Desktop -->
      <div class="navbar-center border-none hidden lg:flex">
        <ul class="menu menu-horizontal px-1 gap-2">
          <li>

            <a href="<?= BASE_URL ?>index.php" class=" font-display font-semibold hover:text-primary transition-colors ">
              <i data-lucide="home" class="size-4"></i>
              Inicio
            </a>
          </li>

          <li>
            <a href="<?= BASE_URL ?>pages/coursesPublic.php" class="font-display font-semibold hover:text-primary transition-colors">
              <i data-lucide="book" class="size-4"></i>
              Cursos
            </a>

          </li>

          <?php if (isset($_SESSION['user'])): ?>
            <li>
              <a href="<?= BASE_URL ?>pages/studentCourses.php" class="font-display font-semibold hover:text-primary transition-colors">
                <i data-lucide="graduation-cap" class="size-4"></i>
                Mi Aprendizaje
              </a>
            </li>
          <?php endif; ?>
        </ul>
      </div>

      <!-- Acciones del Header -->
      <div class="navbar-end gap-2">
        <!-- Theme Toggle -->
        <label class="swap swap-rotate btn btn-ghost btn-sm border-black">
          <input type="checkbox" class="theme-controller" value="night" />

          <!-- Sun icon -->
          <svg class="swap-off fill-current size-5 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">

            <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
          </svg>

          <!-- Moon icon -->
          <svg class="swap-on fill-current size-5 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
          </svg>
        </label>

        <?php if (isset($_SESSION['user'])): ?>
          <!-- Usuario Logueado -->
          <div class="dropdown dropdown-end">
            <label tabindex="0" class="btn btn-ghost btn-circle avatar">
              <div class="w-10 rounded-full ring ring-warning ring-offset-base-100 ring-offset-2">
                <img src="<?= BASE_URL . 'assets/upload/avatars/' . $_SESSION['user']['avatar'] ?>" alt="Avatar" />
              </div>
            </label>
            <ul tabindex="0" class="menu menu-sm dropdown-content mt-3 z-10 p-2 shadow-lg bg-base-100 rounded-box w-52">
              <li class="menu-title">
                <span class="text-xs"><?= $_SESSION['user']['email'] ?></span>
              </li>
              <li>
                <a href="<?= BASE_URL ?>pages/homeProfile.php" class="gap-2">
                  <i data-lucide="user" class="size-4"></i>
                  Mi Perfil
                </a>
              </li>
              <?php if (isset($_SESSION['user']) && $_SESSION['user']['role'] !== 'student') : ?>

                <li>
                  <a href="<?= BASE_URL ?>pages/dashboard.php" class="gap-2">
                    <i data-lucide="layout-dashboard" class="size-4"></i>
                    Dashboard
                  </a>
                </li>
              <?php endif; ?>

              <div class="divider my-1"></div>
              <li>
                <a href="<?= BASE_URL ?>procedures/logout.proc.php" class="text-error gap-2">
                  <i data-lucide="log-out" class="size-4"></i>
                  Cerrar Sesión
                </a>
              </li>
            </ul>
          </div>
        <?php else: ?>
          <!-- Usuario no logueado -->
          <a href="<?= BASE_URL ?>pages/login.php" class="btn btn-outline btn-warning btn-sm gap-2">
            <i data-lucide="log-in" class="size-4"></i>
            <span class="hidden sm:inline">Entrar</span>
          </a>
        <?php endif; ?>
      </div>
    </nav>
  </header>

</body>